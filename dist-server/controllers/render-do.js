...
async function renderDO({ domain: domainName, doNo }) {
    ...
    const productList = foundWSD
        ...
        .reduce((newItem, item) => {
        var foundItem = newItem.find(newItem => newItem.product_name === item.product_name && newItem.product_batch === item.product_batch);
        if (!foundItem) {
            foundItem = {
    ...
    const data = {
        ...
        product_list: productList.map((prod, idx) => {
            return Object.assign(Object.assign({}, prod), { list_no: idx + 1, remark: prod.remark > 1 ? `${prod.remark} PALLETS` : `${prod.remark} PALLET` });
        })
    }; //.. make data from do
